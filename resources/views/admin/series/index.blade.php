...
@section('content')
@php $roles = auth()->user()->roles()->pluck('id')->toArray(); @endphp
                ...
                <table class="table table-striped table-hover table-bordered table-sm mb-2 main-listing-table">
                    <thead>
                        <tr>
                            ...
                            <th style="width:100px">{{ __('admin.text_actions') }}</th>

                                @if(in_array('1', $roles))
                            <th style="width:200px">Organization</th>
                            @endif

                            <th>{{ __('First Number') }}</th>
                            <th>{{ __('Type') }}</th>
                    ...
                    @if($data->count())
                        <tbody>
                            @foreach($data as $item)
                                ...
                                $str = $item['name'] . $item['number_separator'];
                                $result =  $str.str_pad($item['start_number'], $item['min_length'], "0", STR_PAD_LEFT);
                                @endphp
                                <tr>
                                    <td>
                                        <div class="custom-checkbox custom-control">
